...
def redact(latent_img_batch, coords, orig_x_dim, orig_y_dim=0):
    ...
    Parameters:
    - latent_img_batch (4d torch tensor): batch of latent images to be modified
    - coords (tensor): tensor of shape (num_batches, 4) containing x1, y1, x2, y2 for each image
    - orig_x_dim, orig_y_dim (int): dimensions of the original image. If y_dim is zero, assume square image.

    Returns:
    - redacted_latent_img (torch tensor): A version of the latent image with appropriate regions redacted
    """
    if orig_y_dim == 0:
